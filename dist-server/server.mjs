...
function getBearerToken(accessTokenResult) {
  ...
  app.get(["/auth/callback", "/auth/callback/"], async (req, res) => {
    ...
    try {
      ...
      if (!userInfoResponse.ok) {
        throw new Error("No se pudo obtener la informaci\xF3n del usuario de Google.");
      }
  ...
  app.post("/api/calendar/sync", async (req, res) => {
    ...
    try {
      ...
      if (!calendarResponse.ok) {
        ...
        throw new Error(
          `Google Calendar respondi\xF3 ${calendarResponse.status}: ${errorBody || "sin detalle"}`
        );
      }
